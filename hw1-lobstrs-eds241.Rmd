...
```{r}
# plot 1: Total lobster counts by site
# spiny_counts |> 
#     group_by(site, mpa) |> 
#     summarise(total_site_count = sum(counts, na.rm = TRUE)) |> 
# ggplot(aes(site, total_site_count, fill = mpa)) +
# geom_bar(stat = "identity") +
# scale_fill_manual(values = c("MPA" = "darkgreen")) +
# theme_classic()
...
```{r}
# plot 2: Total lobster counts by mpa
# spiny_counts |> 
#     group_by(mpa) |> 
#     summarise(total_mpa_count = sum(counts, na.rm = TRUE)) |> 
# ggplot(aes(mpa, total_mpa_count)) +
# geom_bar(stat = "identity") +
# scale_fill_manual(values = c("MPA" = "darkgreen")) +
# theme_classic()

ggplot(spiny_counts, aes(x = mpa, y = counts)) +
    geom_violin() +
    scale_fill_manual(values = c("MPA" = "darkgreen", "non_MPA" = "grey59")) +
    theme_classic()
```
...
```{r}
# plot 3: Total lobster counts by year
...
```{r}
# plot 4: Total lobster size by mpa
ggplot(spiny_counts, aes(x = mean_size_mm, color = mpa)) +
    geom_density() +
...
spiny_counts |> 
    tbl_summary(by = mpa, include = c(counts))
```
