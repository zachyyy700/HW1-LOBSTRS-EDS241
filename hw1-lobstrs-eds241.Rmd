...
spiny_counts <- spiny_counts |> 
    ...
    mutate(mpa = factor(mpa, levels = c("MPA", "non_MPA")))
...
```{r}
site_order <- c("NAPL", "IVEE", "MOHK", "CARP", "AQUE")
# Plot1: Lobster counts by site
ggplot(spiny_counts, aes(x = counts, y = factor(site, levels = site_order), fill = mpa)) +
    geom_violin(quantile.linetype = "solid") +
    scale_fill_manual(values = c("MPA" = "darkgreen", "non_MPA" = "grey59")) +
    labs(x = "Lobster Count", y = "Site", fill = "MPA Status", title = "Lobster Count Distribution per Site") +
...
ggplot(spiny_counts, aes(x = counts, y = mpa)) +
    ...
    labs(x = "Lobster Count", y = "MPA Status", title = "Lobster Count Distribution by MPA Status") +
...
```{r}
# Plot3: Lobster count jitter by year
medians <- spiny_counts |> 
    group_by(year) |> 
    summarise(median = median(counts, na.rm = TRUE))

ggplot(spiny_counts, aes(x = factor(year), y = counts)) +
    geom_jitter(width = 0.2) +
    labs(x = "Year", y = "Lobster Counts", title = "Lobster Counts Distribution by Year") +

    geom_segment(data = medians, 
    aes(x = as.numeric(factor(year)) - 0.3, xend = as.numeric(factor(year)) + 0.3, y = median, yend = median, color = "Median Count"), 
    linewidth = 1.5) +

    scale_color_manual(values = c("Median Count" = "red")) +
    theme_classic() +
    theme(legend.position = "inside", legend.position.inside = c(0.2, 0.5), legend.title = element_blank(), legend.text = element_text(size= 10), legend.background = element_rect(color = 'gray'))
```
...
ggplot(spiny_counts, aes(x = mean_size_mm, fill = mpa)) +
    ...
    labs(x = "Mean Lobster Size (mm)", y = "Count", fill = "MPA Status", title = "Mean Lobster Size Distribution by MPA Status") +
...
The treatment coefficient represents how treatment changes our counts in 2012, the negative value indicates that treatment sites had lower counts initially. Interpreting the treatment coefficient in the non-link, treatment sites had a 82% reduction in counts. However, this changes over time past 2012 and treatment counts increased indicated by the positive interaction coefficients. This trend from a negative treatment coefficient to a positive takes time (around 2014) indicating MPAs take time to see a difference in lobster counts.
